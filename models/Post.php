...
class Post {
    ...
    // CREATE NORMAL POST
    // ===================================================
public function create($user_id, $content, $imagesJson = null, $pdf = null, $link = null)
{
    $sql = "INSERT INTO posts (user_id, content, images, pdf, link)
            VALUES (?, ?, ?, ?, ?)";

    $stmt = $this->db->prepare($sql);
    return $stmt->execute([$user_id, $content, $imagesJson, $pdf, $link]);
}
    ...
    // UPDATE
    // ===================================================
public function update($id, $user_id, $content, $imagesJson)
{
    $sql = "UPDATE posts SET user_id=?, content=?, images=? WHERE id=?";
    $stmt = $this->db->prepare($sql);
    return $stmt->execute([$user_id, $content, $imagesJson, $id]);
}
